...
`endif
  ...
  logic [$clog2(
NUM_INTERRUPTS
)-1:0] irq_id_instant;  // the interrupt id calculated by id_stage module is sent to cs_register module for mnxti operation
  ...
  assign m_exc_vec_pc_mux_id = ((!CLIC && mtvec_mode == 2'h1) || (CLIC && CLIC_SHV && irq_shv_i))
    ? exc_cause.cause[$clog2(
          NUM_INTERRUPTS
      )-1:0] : {$clog2(
          NUM_INTERRUPTS
      ) {1'b0}};  //TODO CLIC==1 <=> mtvec_mode==2'b11, remove CLIC elab param
  ...
  if (HardwareStacking) begin : gen_hardware_stacking
    rt_ibex_hw_stacking #() hw_stacking_i (
        .clk_i (clk_i),
        .rst_ni(rst_ni),
  ...
  end else begin : gen_no_hardware_stacking
...
`ifdef RVFI
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      rvfi_ext_stage_nmi[0] <= instr_valid_id | ~captured_valid ? irq_nm : captured_nmi;
